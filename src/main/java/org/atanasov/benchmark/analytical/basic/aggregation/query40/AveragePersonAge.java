...
public class AveragePersonAge extends BenchmarkTemplate {

    public static void main(String[] args) throws RunnerException {
        Options opt = new OptionsBuilder()
                .include(AveragePersonAge.class.getSimpleName())
                .forks(1)
                .build();
    ...
    public void prepare() {
        ...
        LOGGER.log(INFO, "DBHITS: {0}",
                profileDbHits(Queries.QUERY_40, 5));
    }

    @Benchmark
    public void query40() {
        driver.session().readTransaction(transaction -> {
            var result = transaction.run(Queries.QUERY_40);

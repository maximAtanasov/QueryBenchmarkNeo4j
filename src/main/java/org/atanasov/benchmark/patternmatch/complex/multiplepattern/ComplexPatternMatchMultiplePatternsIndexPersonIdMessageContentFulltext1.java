...
public class ComplexPatternMatchMultiplePatternsIndexPersonIdMessageContentFulltext1 extends BenchmarkTemplate {

    private long[] personIds;

    public static void main(String[] args) throws RunnerException {
        Options opt = new OptionsBuilder()
                .include(ComplexPatternMatchMultiplePatternsIndexPersonIdMessageContentFulltext1.class.getSimpleName())
                .forks(1)
                .build();
    ...
    public void prepare() {
        ...
        transaction.run("CREATE FULLTEXT INDEX message_content FOR (m:Message) ON EACH [m.content] " +
                "OPTIONS {indexConfig: {`fulltext.analyzer`: 'keyword'}}").consume();
        transaction.commit();
        transaction.close();
